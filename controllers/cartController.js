...
// indCart.items[(findCart.items.length)] = { productId: productId, quantity: quantity }
//     findCart.totalPrice = (findProduct.price * quantity) + findCart.totalPrice
//     ficonst createCart = async (req, res) => {
...
//     fndCart.totalItems = findCart.items.length
//     findCart.save()
            ...
            if (items[i].productId === productId) {
                ...
                if (removeProduct === 1) {
                    if (items[i].quantity === 1 && removeProduct === 1) {
                        ...
                        return res.status(200).send({ status: true, msg: 'sucessfully removed product and cart is empty', data: removedProduct })
                    }
