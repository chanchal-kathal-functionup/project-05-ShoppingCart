...
if (!validate.isValidfeild(title)) {
...
if (!validate.isValidfeild(description)) {
...
if(installments){
 ...
 if(isDeleted){
    ...
    if(data.deletedAt){
        if(deletedAt!="null"){
            return res.status(400).send({ status: false, message: "DeletedAt must be null while creating Product"}); 
        }
    }
} 
...
const getProductById = async (req,res) =>{
 ...
 if(!validate.isValidObjectId(productId)){
  ...
  if(!findProduct){
    ...
    return res.status(200).send({ status: true, message: 'Prouct Details', data:findProduct})
...
const updateProduct=async (req,res) =>{
  ...
  if (!validate.isValidfeild(req.body.title)) {
      return res.status(400).send({ status: false, message: "Invalid format of Title", });
    }
   ...
   if(req.body.description){
       ...
       if (!validate.isValidfeild(req.body.description)) {
          return res.status(400).send({ status: false, message: "Invalid format of description", });
        }
   }
